...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatButtonModule, MatDividerModule, MatFormFieldModule, MatInputModule, MatRadioModule, MatSlideToggleModule, ReactiveFormsModule, RouterLink, FileUploadComponent, MatCardModule],
  ...
  onSubmit() {
    if (this.editSportForm.valid) {
      ...
      this.route.params.subscribe((params) => {
        ...
        this.sportService.editSport(sportId, formData)
          .subscribe(response => {
              console.log('Sport has been successfully edited: ', this.editSportForm.value);
            ...
            (error) => {
              console.error('Sport could not be edited: ', error);
